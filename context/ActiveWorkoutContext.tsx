...
export function ActiveWorkoutProvider({ children }: { children: ReactNode }) {
  ...
  const loadActiveSession = async () => {
    ...
    try {
      ...
      const activeSession = await workoutSessionService.getActiveSession(
        user.id,
      );
      ...
      if (activeSession) {
        ...
        if (hoursSinceStart > 24) {
          ...
          console.log(
            "🧹 [ActiveWorkoutContext] Found stale session from",
        ...
        const sessionData = await workoutSessionService.getSessionWithExercises(
          activeSession.id,
        );
  ...
  const startWorkout = async (templateId: string) => {
    ...
    try {
      console.log(
        "🏋️ [ActiveWorkoutContext] Starting workout from template:",
        templateId,
      );
      console.log(
        "🏋️ [ActiveWorkoutContext] Current session before start:",
        state.sessionId,
      );
      ...
      console.log(
        "🧹 [ActiveWorkoutContext] Cleared stale data - starting fresh",
      );

      // Start workout session (FIX: correct parameter order - userId FIRST!)
      const sessionId = await workoutSessionService.startWorkoutSession(
        user.id,
        templateId,
      );
      ...
      const sessionData =
        await workoutSessionService.getSessionWithExercises(sessionId);
  ...
  const completeWorkout = async () => {
    ...
    try {
      console.log(
        "🏁 [ActiveWorkoutContext] Completing workout session:",
        state.sessionId,
      );

      const duration = state.session?.started_at
        ? Math.floor(
            (Date.now() - new Date(state.session.started_at).getTime()) / 1000,
          )
        : 0;
      ...
      const totalVolume = state.exercises.reduce((sum, exercise) => {
        return (
          ...
          (exercise.sets || []).reduce((setSum, set) => {
            return (
              setSum +
              (set.is_completed ? (set.weight_kg || 0) * (set.reps || 0) : 0)
            );
          }, 0)
        );
      }, 0);

      console.log("📊 [ActiveWorkoutContext] Workout stats:", {
        duration,
        totalVolume,
      });
    ...
    async (
      ...
      setState((prev) => ({
        ...
        exercises: prev.exercises.map((ex) =>
          ex.exercise_id === exerciseId ? { ...ex, status } : ex,
        ),
      }));

      // Update database in background
      try {
        await workoutSessionService.updateExerciseStatus(
          state.sessionId,
          exerciseId,
          status,
        );
  ...
  const updateSetCompletion = useCallback(
    (setId: string, isCompleted: boolean, weight?: number, reps?: number) => {
      ...
      workoutSessionService.updateSet(setId, updateData).catch((err) => {
  ...
  const getExerciseStatus = useCallback(
    (exerciseId: string): "NOT_STARTED" | "IN_PROGRESS" | "COMPLETED" => {
...
export function useActiveWorkout() {
  ...
  // EXPO GO FIX: Return safe fallback instead of throwing
  // Expo Go pre-renders tabs before provider mounts, causing crashes
  if (!context) {
    console.warn(
      "⚠️ useActiveWorkout called before ActiveWorkoutProvider mounted. Returning safe fallback.",
    );

    // Return safe no-op implementation
    return {
      sessionId: null,
      session: null,
      exercises: [],
      ...
      isInitialized: false,
      startWorkout: async () => {
        console.warn("startWorkout called before provider ready");
      },
      cancelWorkout: async () => {
        console.warn("cancelWorkout called before provider ready");
      },
      completeWorkout: async () => {
        console.warn("completeWorkout called before provider ready");
      },
      refreshSession: async () => {
        console.warn("refreshSession called before provider ready");
      },
      updateExerciseStatus: async () => {
        console.warn("updateExerciseStatus called before provider ready");
      },
      updateSetCompletion: () => {
        console.warn("updateSetCompletion called before provider ready");
      },
      getExerciseById: () => undefined,
      getExerciseStatus: () => "NOT_STARTED" as const,
    };
  }
