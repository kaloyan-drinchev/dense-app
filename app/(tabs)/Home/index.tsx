...
import { View, ScrollView, TouchableOpacity, Text, ActivityIndicator, Modal } from 'react-native';
...
export default function HomeScreen() {
  ...
  const {
    // Data
    user, generatedProgram, userProgressData, nextWorkout, workoutAvailability, currentTime,
    // Flags
    loadingProgram, loadingProgress, isRegenerating, isSkipping, isRefreshingWorkout, isWorkoutActive, isRunning,
    // Modals
    ...
    // Handlers
    handleStartWorkoutPress, handleSkipWorkout, handleRegenerateWorkout, 
    handleConfirmWorkout, handleCancelWorkout, handleShareWorkout, handleAdjustDuration,
    pauseTimer, resumeTimer, formatTimerTime, getFirstUncompletedExercise, calculateWorkoutStreak
  } = useHomeLogic();
  ...
  return (
    <SafeAreaView style={styles.container} edges={[]}>
      ...
      >
        ...
        {/* Weekly Schedule */}
        {!loadingProgram && !loadingProgress && generatedProgram && (
          <WeeklySchedule trainingDays={generatedProgram.trainingSchedule || []} />
        )}
            ...
            <View style={styles.workoutCard}>
              ...
              >
                ...
                <View style={styles.workoutHeader}>
                  {isRefreshingWorkout ? (
                    ...
                    <>
                      ...
                      >
                        {nextWorkout.exercises.slice(0, 6).map((exercise: any) => (
                ...
                <View style={styles.actionButtonsRow}>
                  <TouchableOpacity 
                    style={[styles.actionButton, (isWorkoutActive || isSkipping || isRegenerating) && styles.actionButtonDisabled]} 
                    onPress={handleSkipWorkout}
                    disabled={isWorkoutActive || isSkipping || isRegenerating}
                  >
                    {isSkipping ? <ActivityIndicator size="small" color={colors.white} /> : (
                      <>
                  ...
                  <TouchableOpacity 
                    ...
                    disabled={isWorkoutActive || isSkipping || isRegenerating}
                  >
                    {isRegenerating ? <ActivityIndicator size="small" color={colors.white} /> : (
                      <>
                  ...
                  <TouchableOpacity style={styles.actionButton} onPress={handleAdjustDuration}>
                    <Icon name="clock" size={20} color={colors.white} />
                    <Text style={styles.actionButtonText}>Duration</Text>
                  </TouchableOpacity>
                  
                  <TouchableOpacity style={styles.actionButton} onPress={handleShareWorkout}>
        ...
        <View style={styles.workoutsNavigationSection}>
          <View style={styles.workoutsNavigationRow}>
            <TouchableOpacity style={styles.workoutNavCard} onPress={() => router.push('/finished-workouts')}>
            ...
            <TouchableOpacity style={styles.workoutNavCard} onPress={() => router.push('/Programs')}>
          ...
          stats={[
            {
              ...
              infoDescription: 'Total number of days since you started your fitness program.'
            },
            {
              value: userProgressData && generatedProgram ? 
                calculateWorkoutStreak(
                  typeof userProgressData.completedWorkouts === 'string' ? JSON.parse(userProgressData.completedWorkouts) : userProgressData.completedWorkouts, 
            ...
            },
            {
              value: userProgressData && userProgressData.completedWorkouts ? 
                (() => {
                  try {
                    const completed = typeof userProgressData.completedWorkouts === 'string' ? JSON.parse(userProgressData.completedWorkouts) : userProgressData.completedWorkouts;
                    return completed.filter((item: any) => typeof item === 'object' && item.date && item.workoutName).length;
                  } catch { return 0; }
                })() : 0,
      ...
      <HomepageVideoModal visible={showVideoModal} onClose={() => setShowVideoModal(false)} />
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <LinearGradient colors={['#1A1A1A', '#0A0A0A']} style={styles.comingSoonModalContent}>
              <View style={styles.comingSoonIconContainer}>
                <Icon name="star" size={48} color={colors.primary} />
              </View>
              <Text style={styles.comingSoonTitle}>Coming Soon!</Text>
              <Text style={styles.comingSoonDescription}>Our AI-powered custom workout builder is on its way.</Text>
      ...
      {isWorkoutActive && (
        ...
        >
          <LinearGradient
            colors={['rgba(132, 204, 22, 0.15)', 'rgba(34, 197, 94, 0.1)', 'rgba(20, 25, 35, 0.8)']}
                ...
                <View style={styles.bannerLeft}>
                  ...
                  {(() => {
                    ...
                    if (firstExercise) {
                      return (
                        <>
                          <Text style={styles.bannerExerciseName} numberOfLines={1}>{firstExercise.name}</Text>
                  ...
                  >
                    ...
                    >
                      ...
                      <View style={styles.circularPlayButton}>
                        <Icon name={isRunning ? 'pause' : 'play'} size={14} color={colors.black} />
                      </View>
                    </LinearGradient>
                  </TouchableOpacity>
                </View>
