-- ============================================
-- Fix Exercise Name Slug Conversion
-- ============================================
-- This migration fixes the issue where exercise names are stored as slugs
-- (e.g., "leg-curls") instead of proper names (e.g., "Leg Curls") when
-- creating workout sessions from templates.

-- Helper function to convert slug to proper name
CREATE OR REPLACE FUNCTION slug_to_proper_name(slug TEXT)
RETURNS TEXT
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
  ...
  RETURN INITCAP(REPLACE(slug, '-', ' '));
END;
$$;

-- Update the start_workout_session function
...
BEGIN
  ...
  LOOP
    ...
    -- Get exercise name from exercises table (try UUID lookup first)
    SELECT name INTO v_exercise_name
    FROM public.exercises
    WHERE id = v_temp_exercise.exercise_id;

    -- If not found (slug-based ID), convert slug to proper name
    IF v_exercise_name IS NULL THEN
      v_exercise_name := slug_to_proper_name(v_temp_exercise.exercise_id);
    END IF;
    ...
    VALUES (
      ...
      v_exercise_name, -- Now uses proper name from conversion
...
GRANT EXECUTE ON FUNCTION slug_to_proper_name(TEXT) TO authenticated;

-- Add comments
COMMENT ON FUNCTION start_workout_session IS 'Creates a new workout session from a template with fresh state (SECURITY DEFINER to bypass RLS). Converts exercise slug IDs to proper names.';
COMMENT ON FUNCTION slug_to_proper_name IS 'Converts exercise slug (e.g., "leg-curls") to proper name (e.g., "Leg Curls")';
